...
router.get("/createItem", (req, res) => {
    if (req.session.login) {
        ...
        res.send(navbarPage + createItemPage + footerPage);
    } else {
        res.redirect("/login");
    }
});
    ...
    if (req.session.login) {
        ...
        const myItems = await Item.query().
            select(
                ...
                .where("user_id", userId).orderBy("itemId", "ASC");
        res.send({ response: { 
                myItems
            }});    
    } else {
...
router.get("/deleteItem", (req, res) => {
    if (req.session.login) {
        ...
        const footerPage = fileSystem.readFileSync("./public/footer/footer.html", "utf-8");
        res.send(navbarPage + deleteItemPage + footerPage);
    } else {
        res.redirect("/login");
    }
});
...
router.get("/updateItem", (req, res) => {
    if (req.session.login) {
        ...
        const footerPage = fileSystem.readFileSync("./public/footer/footer.html", "utf-8");
        res.send(navbarPage + updateItemPage + footerPage);
    } else {
        res.redirect("/login");
    }
});
...
router.post("/createItem", (req, res) => {
    ...
    if (itemName && brand && category && description && itemAge && price && days) {
        try {
            ...
            res.status(500).send({ response: "Error in database occured."});
        }
    }
});
